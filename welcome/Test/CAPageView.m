...
@interface CAPageView ()

@end

@implementation CAPageView
...
-(void)updateWithViewArray:(NSArray *)array
{
    ...
    for (int i=0;i<count;i++) {
        ...
        if(i==0)
        {
            firstView = [[UIImageView alloc] initWithImage:[CAViewUtils getImageFromView:view]];
        }
        if(i==count -1)
        {
            lastView = [[UIImageView alloc] initWithImage:[CAViewUtils getImageFromView:view]];
        }
    }
